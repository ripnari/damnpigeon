...
export async function findVariantId(product, picked) {
  ...
  if (!v) {
    // retry using raw values (hex colours etc.)
    const alt = {};
  ...
  if (!v) return product.manageVariants ? { error: 'that combination is sold out or unavailable' } : { options: choices };
  if (v.stock && v.stock.inStock === false) return { error: 'that size is sold out' };
  return { variantId: v._id };
}
